 function  Korrelator( e,Imput_Signal )
...
%svnum=1;
%  svnum=[2 6 12 14];                         % номера спутников для поиска
...
if(Imput_Signal==1)%Для модели
 svnum=1;
end
if(Imput_Signal==2)%Для файла с пмехой
 fs=fs /2;   %если используется Гилберт
 svnum=[2 6 12 14];
end
if(Imput_Signal==3)%Для файла без помехи
 fs=fs /2;
 svnum=[2 6 12 14];
end
I = real(data1);
...
figure(2);
mesh(Z);
 xlabel('Фаза кода');ylabel('Сдвиг по Доплеру');zlabel('отношение сигнал шум');
 end;
...
% FftL=1000;
% Fd=fs;
% FftS=abs(fft(data,FftL));% Амплитуды преобразования Фурье сигнала
